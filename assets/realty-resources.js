(() => {
  ...
  tabs.forEach((tab, index) => {
    tab.addEventListener("click", () => selectResource(tab.dataset.resourceTab));
  ...
  const areas = {
    "downtown toronto": { name: "Downtown Toronto", lat: 43.6532, lon: -79.3832, zoom: .195, aliases: ["toronto", "m5v", "m5h"] },
    "north york": { name: "North York", lat: 43.7615, lon: -79.4111, zoom: .16, aliases: ["m2n", "m3n"] },
    "scarborough": { name: "Scarborough", lat: 43.7764, lon: -79.2318, zoom: .18, aliases: ["m1p", "m1s"] },
    "etobicoke": { name: "Etobicoke", lat: 43.6205, lon: -79.5132, zoom: .17, aliases: ["m8v", "m9a"] },
    "mississauga": { name: "Mississauga", lat: 43.5890, lon: -79.6441, zoom: .2, aliases: ["l5b", "l5m"] },
    "port credit": { name: "Port Credit", lat: 43.5516, lon: -79.5856, zoom: .075, aliases: ["l5g"] },
    "brampton": { name: "Brampton", lat: 43.7315, lon: -79.7624, zoom: .2, aliases: ["l6y", "l6x"] },
    "oakville": { name: "Oakville", lat: 43.4675, lon: -79.6877, zoom: .16, aliases: ["l6h", "l6j"] },
    "burlington": { name: "Burlington", lat: 43.3255, lon: -79.7990, zoom: .17, aliases: ["l7l", "l7r"] },
    "hamilton": { name: "Hamilton", lat: 43.2557, lon: -79.8711, zoom: .2, aliases: ["l8p", "l8s"] },
    "st. catharines": { name: "St. Catharines", lat: 43.1594, lon: -79.2469, zoom: .16, aliases: ["st catharines", "l2r", "l2n"] },
    "niagara falls": { name: "Niagara Falls", lat: 43.0896, lon: -79.0849, zoom: .14, aliases: ["l2e", "l2g"] },
  };
  ...
  function findArea(query) {
    ...
    return Object.values(areas).find((area) => {
      ...
      return key === normalized || key.includes(normalized) || area.aliases.some((alias) => alias === normalized || alias.includes(normalized));
    }) || null;
  }
  ...
  function showArea(area) {
    ...
    const south = (area.lat - area.zoom * .72).toFixed(4);
    const north = (area.lat + area.zoom * .72).toFixed(4);
  ...
  searchForm?.addEventListener("submit", (event) => {
    ...
    else if (searchStatus) searchStatus.textContent = "Choose one of the supported Niagara or GTA areas shown in the suggestions.";
  });
  ...
  function calculateTax() {
    ...
    const provincial = graduatedTax(price, [
      [55000, .005], [250000, .01], [400000, .015], [2000000, .02], [Infinity, .025],
    ]);
    const municipal = propertyLocation?.value === "toronto" ? graduatedTax(price, [
      [55000, .005], [250000, .01], [400000, .015], [2000000, .02], [3000000, .025],
      [4000000, .044], [5000000, .0545], [10000000, .065], [20000000, .0755], [Infinity, .086],
    ]) : 0;
    ...
    if (totalTax) totalTax.textContent = currency.format(provincial + municipal);
  }
